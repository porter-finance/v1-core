  ...
  ReentrancyGuard
{
  ...
  function getConvertibilityRatios() external view returns (uint256[] memory) {
  ...
  function mint() external onlyOwner nonReentrant notPastMaturity {
    ...
    for (uint256 i = 0; i < collateralTokens.length; i++) {
      ...
      uint256 collateralDeposited = collateralToken.balanceOf(address(this));
      uint256 backingRatio = backingRatios[i];
  ...
  function unsafeRedeemDefaulted(uint256 bondShares) private {
    ...
    if (borrowingTokenBalance > 0) {
      ...
      uint256 borrowingTokensToReceive = (bondShares * borrowingTokenBalance) /
        totalSupply();
    ...
    for (uint256 i = 0; i < collateralTokens.length; i++) {
      ...
      if (backingRatio > 0) {
        ...
        // external call reentrancy possibility: the bonds are burnt here already - if there weren't enough bonds to burn, an error is thrown
        IERC20(collateralToken).safeTransfer(_msgSender(), collateralToReceive);
  ...
  function sweep(IERC20 token) external nonReentrant {
    if (address(token) == borrowingToken || address(token) == address(this)) {
