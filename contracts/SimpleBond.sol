  ...
  ReentrancyGuard
{
  ...
  modifier defaulted() {
    if (block.timestamp < maturityDate && !isRepaid) {
  ...
  /// @dev CollateralDeposited + Mint events are both emitted. shares to mint is bounded by maxSupply
  /// @param shares the amount of bonds to mint
    ...
    isConvertible
  {
    ...
    emit Converted(_msgSender(), collateralToken, shares, collateralToSend);
  }
  ...
  function previewMint(uint256 shares) public view returns (uint256) {
    return (shares * backingRatio) / ONE;
  }

  function previewConvert(uint256 shares) public view returns (uint256) {
