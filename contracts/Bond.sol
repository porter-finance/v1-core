...
contract Bond {
  ...
  uint256 _coupon;
  uint256 _startDate;
  uint256 _couponsIssued;
  uint256 _couponsToIssue;
  uint256 _interval;
  ...
  function coupon() external view returns (uint256) {
    return _coupon;
  }
  ...
  function couponsIssued() external view returns (uint256) {
    return _couponsIssued;
  }

  function couponsToIssue() external view returns (uint256) {
    return _couponsToIssue;
  }

  function withdraw() external returns (uint256) {
    uint256 _couponsPotentiallyEarned = (block.timestamp - _startDate) /
      _interval;
    uint256 _couponsEarned = _couponsPotentiallyEarned > _couponsToIssue
      ? _couponsToIssue
      : _couponsPotentiallyEarned;
    uint256 _couponsOwed = _couponsEarned - _couponsIssued;
    if (IERC20(_tokenContract).transfer(_recipient, _coupon * _couponsOwed)) {
      ...
      uint256 _availableCoupons = IERC20(_tokenContract).balanceOf(
        address(this)
      ) / _coupon;
